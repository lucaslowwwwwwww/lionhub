...
export default function CustomersPage() {
  ...
  const filteredCustomers = customers.filter(c => {
    ...
    // Check addresses (array of objects or single string)
    const addresses = Array.isArray(c.addresses) ? c.addresses : [c.address || '']
    const addressMatch = addresses.some(a => {
      const val = typeof a === 'object' ? a.value : a
      return val.toLowerCase().includes(searchLower)
    })
    
    // Check phones (array or single string)
    const phones = Array.isArray(c.phones) ? c.phones : [c.phone || '']
    const phoneMatch = phones.some(p => p.includes(searchQuery))
