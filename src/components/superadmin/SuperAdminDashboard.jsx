...
export default function SuperAdminDashboard() {
  ...
  const handleLogout = () => {
    if (impersonatedOrgId) {
      alert("You are currently impersonating an organization. Please click 'Stop Impersonation' on the active tenant card before logging out.")
      return
    }
    logout()
  }
        ...
        <div className="flex items-center gap-6">
          ...
          <button 
            ...
            className={`p-2.5 rounded-xl transition-all border ${
              impersonatedOrgId 
                ? 'bg-surface-900 border-surface-800 text-surface-600 cursor-not-allowed shadow-inner' 
                : 'bg-surface-800 border-surface-700/50 text-surface-400 hover:text-crimson-400 hover:bg-surface-700'
            }`}
            title={impersonatedOrgId ? "Please stop impersonation first to log out" : "Log Out"}
          >
